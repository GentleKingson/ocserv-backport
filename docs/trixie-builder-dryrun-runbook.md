...
- "为什么" 用引用块（`>`）简短说明。
- 验收点用 ✅ 标记，是进入下一步的前置条件。
- 凡占位符（如 `ssh-ed25519 AAAA...`、`<host>`）必须替换为
...
**源码获取模式与锁定身份（`FETCH_SOURCE`）**

构建机通过 `FETCH_SOURCE` 选择源码来源，源码身份由 `source-lock/<name>/<version>.yaml`（Git 追踪的权威）及其 CI 守卫的 `.lock.tsv` projection 锁定：

```bash
cd ~/ocserv-backport
[[ -f .env ]] || cp .env.example .env
# 确认模式（pool = 默认，从 deb.debian.org pool 在线取锁定版本；cache = 读已校验的本地 build/source-cache/）
grep FETCH_SOURCE .env
# 确认 lock projection 存在（CI 守卫它与 YAML 字节一致）
test -f source-lock/ocserv/1.5.0-1.lock.tsv && echo "lock projection present"
```
...
当构建机需要 `FETCH_SOURCE=cache`（例如 pool 不可达、或要在固定时间点重建历史版本）时，cache 由预取节点填充：
...
# 2. 运维层搬运 bundle 到构建机（方式自选：rsync/scp/内部 registry；不脚本化、不硬编码凭据/主机）
...
OCSERV_UPSTREAM_VERSION=1.5.0      （fetch-source.sh 默认；只用于解析 lock path，非身份权威）
OCSERV_DEBIAN_REVISION=1           （fetch-source.sh 默认；只用于解析 lock path，非身份权威）
MAINTAINER_NAME / MAINTAINER_EMAIL （rewrap-changelog.sh 默认）
arch=amd64                          （build-binary.sh --arch=amd64）
```

> 注：这些默认值随脚本演进可能变化，以当前脚本实际默认值为准。`OCSERV_UPSTREAM_VERSION`/`OCSERV_DEBIAN_REVISION` 派生的请求路径若与 `.lock.tsv` 内 META 身份不一致，脚本会显式失败。
...
```bash
grep -qE '^FETCH_SOURCE=(pool|cache)' .env && echo "FETCH_SOURCE set"
test -f source-lock/ocserv/1.5.0-1.lock.tsv && echo "lock projection present"
```

必须输出 "FETCH_SOURCE set" 与 "lock projection present"。
...
|------|-------------|----------|--------|
| 1 | fetch | `build/source/ocserv-1.5.0/` + upstream `ocserv_1.5.0.orig.tar.xz` 及 `.asc` | `FETCH_SOURCE=pool` 从 `deb.debian.org/pool` 在线取锁定版本（强校验闭环 + `dscverify` + `dpkg-source -x`），或 `FETCH_SOURCE=cache` 读已校验版本化 cache；不访问 Snapshot；不自动回退；持久发布 source tree 与 upstream orig tarball(+asc) 到 `build/source/`（src-pkg 的 quilt 重打包需要 orig tarball）。sid 原版 `.dsc` / `.debian.tar.xz` 不发布为 fetch 输出（backport 会重新生成它们） |
...
| 占位符 | 出现位置 | 含义 |
|--------|---------|------|
| `YYYYMMDDTHHMMSSZ` | `source-lock/ocserv/<version>.yaml` 的 `snapshot_timestamp` | snapshot.debian.org 锁定 ocserv 源码的时间戳（已提交真实值，非操作员编辑项；预取节点 prefetch 时消费） |
