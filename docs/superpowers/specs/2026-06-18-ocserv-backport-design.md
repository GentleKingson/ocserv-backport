...
脚本层 flock:
  exec 9>/var/lock/aptly-mutation.lock
...
```text
rclone remote 名称预配置为 "r2", 但不保存 access/secret。
CI runtime 通过环境变量注入:
  R2_ACCESS_KEY_ID
  R2_SECRET_ACCESS_KEY
不把 R2 凭据同时写进 rclone config 和 GitHub secrets, 二选一, 采用环境变量注入。
```
...
on:
  push:
    branches: [main]
    paths: ['debian/**', 'scripts/**', 'Makefile', '.github/workflows/**']

# promote-production.yml / rollback-production.yml
on:
  workflow_dispatch:
    inputs:
      snapshot: { required: true }        # promote / rollback 目标 snapshot
      target_version: { required: true }  # 仅 rollback-production 需要
```
...
pub-prod:        ; scripts/aptly-publish.sh production $(SNAP)
sync-prod:       ; scripts/r2-sync.sh production
purge-prod:      ; scripts/cf-purge.sh production

rollback-testing: ; scripts/aptly-rollback.sh testing $(PREV_SNAP)
rollback-prod:    ; scripts/aptly-rollback.sh production $(TARGET_SNAP)
```

每个 CI job 的 run 块就是一行 `make <target>`。snapshot 名由 `scripts/snapshot-name.sh` 单一产出, CI/Makefile/aptly 三处引用同一脚本, 避免命名漂移。
...
- name: 获取 apt-cache policy
  command: apt-cache policy ocserv
  register: policy_out
  changed_when: false

- name: 断言 candidate 来源与版本正确
  assert:
    that:
      - ocserv_repo_baseurl in policy_out.stdout
      - "'1001' in policy_out.stdout"
      # candidate 版本必须等于目标版本 (解析 apt-cache policy 输出)
    fail_msg: "candidate 不来自 THEHKUS-Backports 当前通道或 priority 异常, 拒绝升级"
...
关键修正: upgrade 不用 `state: latest`, 必须安装 `ocserv={{ ocserv_target_version }}`; 回滚依据是显式传参或 aptly snapshot, 不依赖跨 play register 变量; candidate 校验通过解析 `apt-cache policy` 输出 (不是不存在的 `apt_policy_result.tables`)。
...
### 5.9 verify 任务 (纯只读, TCP+UDP, 配置测试可选)
...
- name: 检查 TCP 监听
  ...
  register: ss_tcp
  changed_when: false
  failed_when: "ocserv_tcp_port | string not in ss_tcp.stdout"

- name: 检查 UDP 监听
  command: ss -lun
  register: ss_udp
  changed_when: false
  failed_when: "ocserv_udp_port | string not in ss_udp.stdout"
...
- name: 检查 journalctl 最近日志无明显 fatal
  command: journalctl -u ocserv --since "5 min ago" --no-pager
  register: journal
  changed_when: false
  failed_when: "'fatal' in journal.stdout or 'config error' in journal.stdout or 'Permission denied' in journal.stdout"
...
- name: occtl 可执行 (以 root 或有 socket 权限用户)
  command: occtl show users
  register: occtl_out
  changed_when: false
  become: yes
  failed_when: "'No such file' in occtl_out.stderr"
```

verify 是纯只读、可独立调用: CI 的 staging-upgrade job 调它、prod 升级后人工调它、日常巡检也调它。端口检查同时覆盖 TCP 和 UDP; `ocserv --test-config` 设为可选 (参数是否存在取决于版本和打包方式); occtl 明确以 root 执行 (需访问管理 socket)。
...
[1] 基础构建工具链
    apt install -y \
      sbuild schroot sbuild-schroot debootstrap \
