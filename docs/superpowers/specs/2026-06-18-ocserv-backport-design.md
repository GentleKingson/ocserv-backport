...
- 状态: 已确认 v3 (实现细节修正已并入, 待 writing-plans)
- 日期: 2026-06-18
...
```text
snapshot 名: ocserv-1.5.0-1~bpo13+1-build-<buildno>
  - 含版本号 + build 号, 可追溯
  - 不含 testing/prod (snapshot 是构建产物, 与发布通道解耦)
```

**build 号来源 (避免同 commit 重跑撞名):**

```text
CI:    GITHUB_RUN_NUMBER (单调递增, 如 gh123)
       命名: ocserv-1.5.0-1~bpo13+1-build-gh123
本地:  local-<timestamp>
       命名: ocserv-1.5.0-1~bpo13+1-build-local-20260618T120000
```

不用 `git rev-parse --short HEAD` 作 build 号: 同一 commit 重跑 CI 会生成相同 snapshot 名, 导致 `aptly snapshot create` 撞名失败或需大量幂等分支处理。`scripts/snapshot-name.sh` 检测运行环境 (CI vs 本地) 自动选择来源。
...
snapshot 名:   ocserv-1.5.0-1~bpo13+1-build-gh123  (CI; build 号见 §2.2)
```
...
snapshot 名:   ocserv-1.5.0-1~bpo13+1-build-gh123        (CI, GITHUB_RUN_NUMBER)
              ocserv-1.5.0-1~bpo13+1-build-local-20260618T120000  (本地 dry-run)
CI artifact:   ocserv-1.5.0-1~bpo13+1-build-gh123.tar
```
...
aptly repo:     ocserv-backports        (单一收包池, 可变, 可含多个历史版本)
                        ...
                        │ aptly repo add *.deb
                        ↓
aptly snapshot: ocserv-1.5.0-1~bpo13+1-build-gh123  (不可变, 构建产物固化)
aptly snapshot: ocserv-1.5.0-1~bpo13+2-build-gh157  (不可变)
                        │
          ┌─────────────┴──────────────┐
          │                            │
...
| 操作 | aptly 命令 | 后续 | 触发 |
|------|-----------|------|------|
| 首次发 testing | `snapshot create` → `publish snapshot ... trixie-testing` | rclone sync /testing/ + purge + 更新 manifest | CI 自动 |
| testing 升级新版本 | `repo add` → `snapshot create` → `publish switch trixie-testing <new>` | rclone sync /testing/ + purge + 更新 manifest | CI 自动 |
| staging 自动回滚 | `publish switch trixie-testing <previous-good>` | rclone sync /testing/ + purge + ansible rollback (version ← manifest) | 探活失败自动 |
| 首次 promote 到 prod | `publish snapshot ... trixie-production` | rclone sync /prod/ + purge + 更新 manifest | **人工 workflow_dispatch** |
| 后续 promote 到 prod | `publish switch trixie-production <validated>` | rclone sync /prod/ + purge + 更新 manifest | **人工 workflow_dispatch** |
| prod 人工回滚 | `publish switch trixie-production <previous-good>` | rclone sync /prod/ + purge + ansible rollback (version ← manifest) | **人工确认** |
...
```text
每通道首次发布:  publish snapshot <snap> <distribution>
                (trixie-testing / trixie-production 当时还不存在)
通道已存在后:    publish switch <distribution> <snap>
                (更新/回滚, 原地切换 snapshot)

aptly-publish.sh 必须检测目标通道是否已 publish:
  1. aptly publish list 查询 distribution 是否存在
  2. 不存在 → aptly publish snapshot (首次)
  3. 已存在 → aptly publish switch (后续)
```

这对 `/prod/` 尤其重要: 第一次 promote production 必然是首次发布, 直接用 `publish switch` 会失败。

### 3.8 publish 状态 manifest (snapshot ↔ version 映射)

rollback 需要同时知道 previous-good snapshot 和它对应的 deb 版本 (Ansible rollback 要显式传 `ocserv_target_version`)。否则会出现"仓库已切回旧 snapshot, 但主机 rollback 不知道装哪个版本"的断点。

`scripts/aptly-publish.sh` 在每次 publish switch 成功后, 更新构建机的状态文件:

```text
/var/aptly/state/testing-current.json
/var/aptly/state/testing-previous-good.json
/var/aptly/state/production-current.json
/var/aptly/state/production-previous-good.json
```

示例内容:
```json
{
  "snapshot": "ocserv-1.5.0-1~bpo13+1-build-gh123",
  "version": "1.5.0-1~bpo13+1",
  "channel": "testing",
  "promoted_at": "2026-06-18T12:00:00Z"
}
```
...
```text
publish switch 成功后:
  previous-good.json ← 旧的 current.json
  current.json       ← 新 snapshot + 版本 + 时间戳
```

**rollback 读取规则:**
```text
staging 自动 rollback:
  读 testing-previous-good.json
  aptly switch testing → previous_good.snapshot
  ansible rollback -e ocserv_target_version=previous_good.version

prod 人工 rollback:
  读 production-previous-good.json (或在 workflow_dispatch 显式传 snapshot+version)
  aptly switch production → previous_good.snapshot
  ansible rollback -e ocserv_target_version=previous_good.version
```

state 目录 owner = builder, 归入 §6.1 备份清单。
...
│ aptly publish snapshot/switch trixie-testing  │
│   (首次 snapshot, 后续 switch; 见 §3.8)       │
│ rclone sync → R2 /testing/                    │
│ Cloudflare purge /testing/dists/*             │
│ 更新 /var/aptly/state/testing-*.json manifest │
...
│ aptly publish snapshot/switch trixie-production│
│   (首次 snapshot, 后续 switch; 见 §3.8)        │
│ rclone sync → R2 /prod/                        │
│ Cloudflare purge /prod/dists/*                 │
│ 更新 /var/aptly/state/production-*.json manifest│
│ ❌ 不自动升级生产机, 由独立人工动作触发          │
└─────────────────────────────────────────────────┘
...
└── files/
    ...
    └── assert-apt-policy.sh         # apt-cache policy 校验 (role 自包含)
```
...
- name: 校验 candidate 来源/版本/priority (调用 role 自带脚本, 解析 apt-cache policy)
  script: >
...
- candidate 校验由 role 自带脚本 `roles/ocserv_backport/files/assert-apt-policy.sh` 解析 `apt-cache policy ocserv`, 不留给实现者自由发挥 (见 5.11)
...
- name: 检查版本号匹配预期 (统一用 ocserv_target_version)
  command: dpkg-query -W -f='${Version}' ocserv
  register: ver
  changed_when: false
  failed_when: >
...
`assert-apt-policy.sh` 放在 role 内部 `roles/ocserv_backport/files/assert-apt-policy.sh` (而非项目根 `scripts/`), 因为 Ansible `script:` 模块从控制端复制脚本到远端, 放 role/files 使 role 自包含, 不依赖仓库相对路径。

脚本解析 `apt-cache policy ocserv` 输出, 验收 candidate 来源/版本/priority。
...
[5] 本地 publish 目录 + state 目录
    ...
    mkdir -p /var/aptly/state          # publish manifest (snapshot ↔ version)
    chown -R builder:builder /var/aptly/state
...
[10] 备份 (构建机有持久状态, 必须备份)
    ...
    /var/aptly/state            (publish manifest: snapshot ↔ version 映射)
    ~/.gnupg                    (仓库签名能力)
...
│  断言: CI 输出 = ocserv-1.5.0-1~bpo13+1-build-gh<n>│
│        本地 输出 = ocserv-1.5.0-1~bpo13+1-build-local-<ts>│
...
│   ├── aptly-publish.sh           # testing/prod; 检测首次 publish vs switch; 维护 state manifest
│   ├── aptly-rollback.sh
...
| `build-gh123` 中的 `gh123` / `local-20260618T120000` | snapshot 名 / CI artifact | `scripts/snapshot-name.sh` 运行时: CI 用 `GITHUB_RUN_NUMBER` 拼成 `gh<n>`, 本地用 `local-<timestamp>`。不取 git short SHA (避免同 commit 重跑撞名) |
...
### v3 (实现细节修正, 2026-06-18)

按 v2 复审反馈并入 5 点实现细节, 减少实现阶段返工:

```text
1. snapshot build 号固定来源: CI 用 GITHUB_RUN_NUMBER (build-gh123),
   本地 dry-run 用 local-<timestamp>; 不用 git short SHA (避免同 commit 重跑撞名)
2. aptly-publish.sh 检测目标通道是否已 publish: 首次 publish snapshot,
   后续 publish switch (production 首次 promote 必然走 snapshot 分支)
3. publish 状态 manifest (/var/aptly/state/*-{current,previous-good}.json)
   维护 snapshot ↔ version 映射, 解除 rollback 时 "仓库已切但不知道装哪个版本" 的断点
4. verify 版本断言变量统一为 ocserv_target_version (原 expected_version 未传入会静默失效)
5. assert-apt-policy.sh 放入 role/files 使 role 自包含,
   Ansible script 模块用 folded scalar + files/ 相对路径
```
