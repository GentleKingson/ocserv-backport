...
"当时用什么信任根验证了这个 `.dsc`"。`dscverify_keyrings` 是**有序数组**,完整记录
advisory A 实际传入的全部 keyring(默认四份);不把 schema 锁死为两份。prefetch 在
§3.6 step 4 计算并填入:
...
3. 校验 bundle 整体 sha256(修正 #5 — 严格 sidecar 解析, 不让未受信 sidecar 作任意输入):
   ...
   解析成功后, 脚本自行构造单行 stdin 做 checksum:
     printf '%s  %s\n' "$expected_hash" "$bundle_basename" | sha256sum --check -
   (不把未受信 sidecar 文件直接喂给 sha256sum -c。)
