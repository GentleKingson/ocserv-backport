...
```json
{
  ...
  "provenance": {
    ...
    "verification": {
      "dscverify_version": "...",
      "dpkg_source_version": "...",
      "debian_keyring_path": "/usr/share/keyrings/debian-keyring.gpg",
      "debian_keyring_sha256": "...",
      "debian_keyring_package_version": "...",
      "tag2upload_keyring_path": "/usr/share/keyrings/debian-tag2upload.pgp",
      "tag2upload_keyring_sha256": "...",
      "tag2upload_keyring_package_version": "..."
    }
  }
}
```
...
- `*_keyring_path`:`/usr/share/keyrings/debian-keyring.gpg` /
  `/usr/share/keyrings/debian-tag2upload.pgp`(advisory A 实际传入的路径)
- `*_keyring_sha256`:对应 keyring 文件的 sha256
- `*_keyring_package_version`:`dpkg-query -W -f='${Version}' debian-keyring` /
...
4. provenance 自算:
   ...
   - verification(修正阻塞项 #4): dscverify/dpkg-source 版本、实际传入的两个
     keyring 路径 + 其 sha256 + 其 dpkg-query 包版本(见 §3.4 provenance.verification)
...
5. tar 类型预扫描(解压前; 修正阻塞项 #2):
   LC_ALL=C tar --list --verbose --zstd --file "$bundle"
...
6. 仅在空 staging 目录(mktemp -d)中解压, 使用受限选项:
     LC_ALL=C tar --extract --zstd --file "$bundle" --directory "$staging" \
...
**import 端对应(§3.7 步骤 5/6):** `--zstd` 显式指定,不依赖 tar 对 `.tar.zst` 的
隐式识别;`LC_ALL=C` 保证 verbose 首字段(mode/type)按固定字节解析。GNU tar 对
hard link 用独立 typeflag 表示;显式限定只接受 regular file(首字符 `-`)最容易审计。
...
- bundle 创建必须是 regular-file-only + ustar + --no-recursion + LC_ALL=C
  (断言:用 `tar --list --verbose` 检查生成 bundle 无目录成员/无 link/PAX 头)
...
- tar 预扫描必须用 `--list --verbose --zstd` 且 `LC_ALL=C`(断言)
- 解压结构错 → die
- 解压必须用 `--zstd --no-same-owner --no-same-permissions --no-overwrite-dir`
  且 `LC_ALL=C`(断言)
...
--require-strong-checksums`;pool 模式还执行 `dscverify`。因此构建机(不只是预取
节点)必须有对应运行时依赖。**这些必须在 runbook 的构建机前置检查中体现,不只是
预取节点章节。**

```
FETCH_SOURCE=pool:                     FETCH_SOURCE=cache:
  curl 或 wget(实现中二选一并固定)        dpkg-dev
...
- pool 模式需要 `devscripts`(提供 `dscverify`)+ keyring + 网络下载器(curl/wget,
  实现固定其一,不两种都调)。
- cache 模式不需要 `dscverify`(§4.4:cache 的 `.dsc` 已在 prefetch 阶段验签),
  但**仍需** `dpkg-dev` + keyring —— 因为 `dpkg-source --require-valid-signature`
  会用 keyring 重新验证 `.dsc` 签名。
- `debian-keyring` 提供 `/usr/share/keyrings/debian-keyring.gpg`;
  `debian-tag2upload-keyring` 提供 `/usr/share/keyrings/debian-tag2upload.pgp`
  (Trixie 实际路径)。两者随系统升级更新内容(见 §3.6 阻塞项 #4 的 provenance 记录)。
...
3. 版本断言: meta_format_version == 1; manifest_schema_version == 1
   (后者是 cache.meta 镜像值, consumer compatibility declaration, 见 §3.5.1)
