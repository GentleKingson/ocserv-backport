...
- 状态: 已批准 v1.2.1 (Phase 0 最终架构约束文档; 非阻塞性文字一致性已并入)
- 日期: 2026-06-21
...
deploy-staging 容器:
  出: staging hosts(TCP 22) | control-plane-testing(读 immutable plan + 提交绑定 result, 无 publish 权, §4.7)
  入: 无

deploy-production 容器:
  出: production hosts(TCP 22) | control-plane-production(读 immutable plan + 提交绑定 result, §4.7)
  入: 无
...
| staging deploy | `staging` | `deploy-staging` | 不调用 publish API (读 immutable plan + 提交绑定 result, §4.7) |
| production publish | `production` | `publish-production` | production promotion/rollback API |
| production deploy | `production` 或单独 `production-deploy` | `deploy-production` | 不调用 aptly/publish API (读 immutable plan + 提交绑定 result, §4.7) |
...
### v1.2.1 (2026-06-21, 非阻塞性文字一致性)

Phase 0 批准后补的交叉表述同步: §1.4 网络策略与 §5 映射表中 deploy-staging / deploy-production
仍写"仅查询 / 仅 GET plan", 未同步到 §4.2/§4.7 的更新 (deploy runner 现可读 immutable plan +
提交绑定 result, 不自行完成 operation)。不改变权限模型, 不阻塞 Phase 1。4 处表述已统一。
