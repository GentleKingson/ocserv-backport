...
- 状态: 已确认 v1.1 (评审 7 项阻塞收口 + 一致性修正已并入, 待 writing-plans)
- 日期: 2026-06-21
...
[published]             aptly publish switch <dist> <snapshot>         ← GPG 签名在此步发生
        ...
        ▼
[externalizing]         受控分阶段外部提交算法 (§3.2.1), 非裸 rclone sync
        │               → pool 对象先传, metadata 次之, InRelease 最后作为可见性提交标记
        ▼
[r2_synced]             externalization 完成 + 独立 probe 校验 (§3.2.1 step 6)
...
[cdn_purged]            CF purge <channel>/dists/*
        ...
        ▼
[completed]             channel_state_record_update(current/previous-good)   ← 最后才写
...
**受控分阶段外部提交算法 (worker 在 `externalizing` 状态执行):**

```text
1. 先上传不可变 package / pool objects (按 sha256 命名或保持 aptly pool 路径);
2. HEAD 或重新下载校验每个对象与 local expected hash 一致 (上传后立即验证, 不信任上传成功返回值);
3. 上传 Packages / Packages.gz / Release.gpg 等 metadata (非 InRelease);
4. 再次验证 metadata 引用的 package closure 已全部存在于 R2 (无悬空引用);
5. 最后上传 / 覆盖 InRelease 作为外部可见提交标记 (InRelease 是 client 解析的入口, 必须最后落盘);
6. CDN purge 后, 从独立 probe (非 publish host 自身网络) 读取并验证:
     InRelease 签名有效、Packages 与 target closure 一致、package bytes 可下载且 sha256 匹配;
7. 旧 package / pool object 不得由常规 sync 立即删除;
8. 旧对象只能由 retention / GC (§2.6) 在确认没有已发布 metadata、rollback target 或仍活跃
   client 引用后删除。
```
...
同一 channel 存在未完成的 externalization transaction 时 (状态处于 `published`..`completed`
之间, 或处于可恢复阻塞态), 必须满足:
...
|---|---|---|---|
| `POST /v1/session` | 任意 Job (仅 server TLS, 无 client mTLS; OIDC 唯一凭证) | bootstrap: 验证 OIDC 后签发短期 mTLS client certificate (§4.3) | OIDC JWT, CSR/一次性公钥, nonce |
| `POST /v1/candidate-releases` | ci-build (mTLS, OIDC; **仅 control-plane-testing 暴露此端点**) | 创建 candidate release, 返回 release_id + 上传流程 | OIDC token, repo/ref/commit/run/attempt |
| `POST /v1/candidate-releases/{release_id}/manifest` | ci-build (**仅 testing**) | 提交 canonical manifest, control plane 校验后签发 PUT URL | manifest JSON (§2.3) |
...
(共 9 个端点: 1 bootstrap + 3 candidate-ingress + 2 publish/promote + 1 rollback + 2 GET。)
...
- endpoint 与 workflow role 的精确匹配 (见 §4.4)。
...
|---|---|---|
| bootstrap (任意 Job 获取 mTLS cert) | `publisher-bootstrap` | repository_id 固定、repository_owner_id 固定、iss 固定; 后续 role 由 workflow_ref/event/environment 二次约束 |
...
| deploy-production-read | `publisher-deploy-production-read` | 仅 deploy workflow、仅 production environment、仅 GET deploy/rollback plan |
...
□ 4. control plane API 契约 (9 端点含 bootstrap + 状态机)        → §4
...
□ 9. publish/promote 状态机 + 外部可见性提交算法 (§3.2.1)        → §3.2, §4.1
□ 10. 受控两阶段 rollback 状态机                                 → §8
□ 11. 并发锁/幂等/审计/失败恢复 (含 externalization lock §3.2.2) → §3.3, §3.4, §9, §10
