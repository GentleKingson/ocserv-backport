...
**Explicitly NOT in Phase 1:** candidate release API, mTLS bootstrap, R2 staging, Sigstore, aptly, GPG, publish host, testing publish, staging deploy, production promotion, rollback control plane, production credentials, JIT, autoscaler/timer/pool, FQDN egress proxy, **IPv6 support on the managed runner path / project-managed IPv6 firewall chains**.
