...
write_deb() {
  local path="${SMOKE_REPO}/build/binary/${1:-ocserv_1.5.0-1~bpo13+1_amd64.deb}"
  printf 'fake deb\n' > "${path}"
}
...
@test "smoke-basic validates local deb metadata and passes expected values to container" {
  ...
  teardown_smoke_repo
  [ "${status}" -eq 0 ]
  [[ "${args}" == *"ocserv_1.5.0-1~bpo13+1_amd64.deb"* ]]
  [[ "${args}" == *"1.5.0-1~bpo13+1"* ]]
  [[ "${args}" == *"amd64"* ]]
}
